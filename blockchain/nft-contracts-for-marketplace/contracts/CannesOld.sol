...
import "../utils/token/ERC721/extensions/ERC721Enumerable.sol";
import "../utils/token/ERC721/extensions/ERC721URIStorage.sol";
import "../utils/access/AccessControlEnumerable.sol";
import "../utils/token/ERC20/IERC20.sol";
import "../utils/utils/introspection/ERC2981.sol";

contract CannesOld is
    ERC721Enumerable,
    ERC2981,
    ERC721URIStorage,
    AccessControlEnumerable
{
    ...
    // lock the contract for a period of time
    function lock(uint256 _timeStamp) public {
        // check message.senders is the contract owner
        hasRole(ROYALTY_UPDATER_ROLE, _msgSender());
  
        _lockedUntil = block.timestamp + _timeStamp;
    }
    ...
    //! When we min the token, we can set royalty address and royalty percentage
    //! I a transfer happens, the tranfer function will take a value (price)
    //! and calculate the royalties and will send to the royalty address
    //! if opensea does this tranfer, they will not setup price and royalty address will not receive any royalties
